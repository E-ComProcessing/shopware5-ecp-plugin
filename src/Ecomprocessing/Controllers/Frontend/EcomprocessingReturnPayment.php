...
/**
 * Class Shopware_Controllers_Frontend_EcomprocessingCheckoutPayment
 * @SuppressWarnings(PHPMD.CamelCaseClassName)
 */
//@codingStandardsIgnoreStart
class Shopware_Controllers_Frontend_EcomprocessingReturnPayment extends FrontendPaymentAction implements CSRFWhitelistAware
    ...
    public function returnAction()
    {
        ...
        $parameters = [
            'transactionId' => $this->Request()->getParam(
                EcomprocessingPaymentAttributes::RETURN_ACTION_PARAM_TRANSACTION_ID
            ),
            'token'         => $this->Request()->getParam(
                EcomprocessingPaymentAttributes::RETURN_ACTION_PARAM_TOKEN
            ),
            'status'        => $this->Request()->getParam(
                EcomprocessingPaymentAttributes::RETURN_ACTION_PARAM_STATUS
            )
        ];
        ...
        switch ($parameters['status']) {
            case EcomprocessingPaymentAttributes::RETURN_ACTION_STATUS_SUCCESS:
                // The Order States will be handled from the Notification Logic
                $this->logger->info(
                    'Received Success Return URL from Genesis',
                    sprintf('ecomprocessing_%s', $transaction->getPaymentMethod()),
                    $parameters
                );
                ...
                break;
            case EcomprocessingPaymentAttributes::RETURN_ACTION_STATUS_CANCEL:
                $this->logger->info(
                ...
                break;
            case EcomprocessingPaymentAttributes::RETURN_ACTION_STATUS_FAILURE:
                $this->logger->info(
